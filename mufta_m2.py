...
print(result)
if result:
    ffa.write_to_file_m2(diameter_body, thickness_body, strength_class, material_korp, temporary_resistance,
                         yield_strength, relative_elongation, temp_kcv, kcv_value, temp_kcu, kcu_value, equiv1, equiv2,
                         one_mass, climatic_version, thickness_shell)
    #clipboard.copy(f"{name(diameter_body, thickness_body, strength_class, pressure, working_coefficient)}\n"
     #              f"""{atvi_korpusa(material_korp, thickness_body, strength_class, temporary_resistance,
      #                                yield_strength, relative_elongation, temp_kcv, kcv_value, temp_kcu, kcu_value,
       #                               equiv1, equiv2, length, one_mass)}\n{mufta_bottom(diameter_body)}\n """
        #           f"{balka_plug(diameter_body, thickness_shell)}\n"
         #          f"{balka(diameter_body, thickness_shell)}")
